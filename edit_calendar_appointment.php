...
try {
    // Database connection
    $conn = new PDO('mysql:host=127.0.0.1:3306;dbname=calendar_db', 'root', 'manicquail735');
    $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

    // Check if appointment ID is provided
    if (isset($_GET['appointment_id'])) {
        $appointmentId = $_GET['appointment_id']; // Retrieve the appointment ID from the URL

        // Fetch appointment details from the database
        $stmt = $conn->prepare("SELECT * FROM appointments WHERE id = :appointmentId");
